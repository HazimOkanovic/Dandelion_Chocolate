...
test.skip('Truffle making events test', {tag: '@regression'}, async () => {
    await expect(await page.url()).toMatch(PagesUrls.eventsPage)
    let eventsPage = new EventsPage(page);
    let allEvents = [];
    for (let index = 0; index < await eventsPage.truffleMakingEvents.count(); index++) {
        const element = await eventsPage.truffleMakingEvents.nth(index);
        allEvents.push(element);
    }
       
    for (const event of allEvents) {
        await event.click();
        expect(await eventsPage.modalHeader).toHaveText(Data.truffleMakingHeader);
        const [newPage] = await Promise.all([
            context.waitForEvent('page'), 
            await eventsPage.bookNowButton.click()
            ])
        await newPage.waitForTimeout(13000);
        await expect(newPage.locator("//div[@class='Consumer-reservationInfo']//h3[contains(text(), 'Making')]")).toHaveText(Data.truffleMakingHeader);
